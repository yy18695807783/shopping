...
public class NewPostListViewAdapter extends BaseAdapter {
    ...
    private List<NewPostBean.ResultBean> result;//  数据来源   http://192.168.1.144:8080/atguigu/json/NEW_POST_URL.json
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (comment_list != null && comment_list.size() > 0) {
            ...
            Collections.shuffle(comment_list);//弹幕数据信息
            holder.danmakuView.addItem(list, true);
            holder.danmakuView.show();//显示
        }else{
    ...
    static class ViewHolder {
        ...
        DanmakuView danmakuView;//弹幕视图组件
